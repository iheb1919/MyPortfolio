...
import { Whatsapp, Youtube, Facebook, ArrowDiagonalTopRight } from '../../assets/iconsSVG/svgIcons';
...
const FooterContact = () => {
  ...
  return (
    ...
    <div className=" mx-auto container">
        ...
        <div className='text-md dark:text-white/50 text-center  py-[40px] my-[40px] border-t border-[#222] ' >©2025  coded with love By <span className='text-[var(--text-color)]   underline decoration-1 font-bold decoration-primary underline-offset-1' >  MEJRI Iheb</span> </div>
    </div>
    </section>
  )
}
...
export const FooterElem: React.FC<FooterElemProps> = ({ title, text, footer, list,styles }) => {
  ...
  return (
    <div style={{...styles}} className="pr-4 last:pr-0 text-lightGray dark:text-lightGrayText max-md:!ml-0   ">
      <div className="flex flex-col gap-4">
        {title && <div className="text-xl font-bold mb-4">{title}</div>}
        <div className="flex flex-col gap-4">
          {text && <p className=" text-xs leading-6">{text}</p>}
          {isListArray && (
            <div>
